...
async def init_rag_components(args, rag_mode, log_callback):
    """
    初始化RAG组件
    """
    ...
    # 检查是否已经初始化
    if 'router' in rag_components:
        return
    
    await log_callback("初始化RAG组件...")
    ...
    # 读取索引
    raptor_index = advanced_indexer.read_index(persist_directory="./chroma_db")
    iterative_retriever = IterativeRetriever(raptor_index, args.model_name, args.base_url, args.embedding_name)

    await log_callback("RAG组件初始化完成")

    # 更新记忆中的组件
    rag_components['indexer'] = advanced_indexer
    rag_components['retriever'] = iterative_retriever
    rag_components['router'] = agentic_router
    rag_components['transformer'] = query_transformer
    rag_components['evaluator'] = self_evaluator
...
async def run_consistency_pipeline(text: str, args, log_callback, **kwargs):
    ...
    for i, chunk in enumerate(chunks):
        ...
        if use_rag:
            ents = await extract_entities_with_rag(entity_extract_chain, chunk_input, log_callback)
        else:
...
async def extract_entities_with_rag(chain, text, log_callback):
    """
    使用RAG辅助提取实体
    """
    global rag_components
    ...
    try:
        # 1. 检查是否有RAG组件
        if 'router' not in rag_components:
            from llm.entity import extract_entities
            return extract_entities(chain, text)

        router = rag_components['router']
        ...
        if "retrieve" in decision.lower():
            # 执行检索
            docs = retriever.retrieve(text, max_iterations=2)
...
"""
            ...
            # 使用增强后的文本提取实体
            from llm.entity import extract_entities
            return extract_entities(chain, enhanced_text)
...
async def run_grammar_pipeline(text: str, args, log_callback, **kwargs):
    ...
    for chunk in chunks:
        ...
        use_rag = rag_mode == "auto" and rag_enabled
        if use_rag:
            result = await run_grammar_check_with_rag(grammar_check_chain, chunk, log_callback)
        else:
...
async def run_grammar_check_with_rag(chain, text, log_callback):
    """
    使用RAG增强语法检查
    """
    global rag_components
...
async def websocket_chat(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # 加载或初始化RAG组件
            if rag_enabled:
                await init_rag_components(args, rag_mode, log_callback)
            else:
