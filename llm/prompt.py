...
## 任务
你的任务不是简单寻找差异，而是判断：
在给定上下文中，一个实体的所有属性、事件和关系是否构成“不可接受的一致性冲突”。

## 工作流程
在分析时，你必须遵循以下三步流程，且不可跳步：
### 第一步：事实聚合
- 汇总该实体在文本中出现的所有属性、事件、状态和关系描述。
- 不要立即下结论，不要提前判断冲突。
- 对同一属性的不同表述，全部保留为候选事实。
### 第二步：冲突候选检测
识别可能存在矛盾的候选对，包括但不限于：
- 属性冲突（如：产地=日本 vs 产地=中国）
- 类型冲突（如：发动机 vs 软件组件）
- 状态冲突（如：罪犯 vs 卧底警察）
- 时间冲突（如：2019发布 vs 2020才设计）
- 数值冲突（如：功率=100W vs 150W）
- 事件因果或描述冲突
- 与其他实体的关系冲突

此阶段只标记“潜在冲突”，不要判断是否成立。
### 第三步：规则层裁决
在确认冲突之前，必须检查以下【允许不一致规则】。
若满足任一规则，则该候选冲突应被【豁免】，而不是直接判为冲突。

【允许不一致规则包括但不限于】：

1. 自我修正规则（self_correction）
- 后文明确修正、否定或替代前文说法
- 修正主体一致，且语义明确
- 示例：“更正一下”“后来发现”“实际上应为”

2. 引用陈述规则（quoted_statement）
- 冲突内容属于引用、转述、报道或他人观点
- 作者立场未明确采信该说法
- 示例：“有人认为…”“报道称…”

3. 时间演进规则（time_progression）
- 不同描述对应不同时间阶段
- 属性或状态随时间合理变化
- 示例：“早期版本”“后来升级为”

4. 视角差异规则（perspective_difference）
- 不同角色、机构或视角给出的描述
- 未构成事实层面的直接否定

## 注意事项
- 若所有候选冲突均被规则层豁免，则 has_conflict = false
- 若存在未被规则豁免的高严重度冲突，则 has_conflict = true
- 只要存在候选冲突，无论是否被豁免，conflicts 列表中必须包含该冲突，但指明是否被豁免。
...
{{
  ...
  "conflicts": [
    {{
      ...
      "conflict_items": [],
        "severity": "high",
        "rule_applied": "none | self_correction | quoted_statement | time_progression | perspective_difference",
        "status": "confirmed | mitigated | unresolved",
        "reason": ""
    }}
  ],
  "explanation": ""
